...
import api from '../../api';
import { Plus, Edit2, Trash2, User, X, Users as GroupsIcon, ChevronDown, Search, Check } from 'lucide-react';
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-visible animate-in zoom-in duration-200 border dark:border-slate-800">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
              ...
              <div className="relative">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Assign Groups</label>
                ...
                >
                  <span className={selectedGroupIds.length === 0 ? "text-slate-400" : "text-slate-900 dark:text-white"}>
                ...
                {isDropdownOpen && (
                  <div className="absolute z-10 w-full mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
                      {groups
                        ?.filter(g => g.name.toLowerCase().includes(groupSearch.toLowerCase()) || g.description.toLowerCase().includes(groupSearch.toLowerCase()))
                        .map(group => (
                        <div
                          key={group.id}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-[10px] opacity-70">{group.description}</div>
                          </div>
                        </div>
                      ))}
                      {groups?.filter(g => g.name.toLowerCase().includes(groupSearch.toLowerCase()) || g.description.toLowerCase().includes(groupSearch.toLowerCase())).length === 0 && (
