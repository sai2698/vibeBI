...
import api from '../../../api';
import { X, Save, Type, Hash, Calendar, ToggleLeft, ToggleRight, FileText, Code, AlertCircle } from 'lucide-react';
    ...
    <div className="w-80 shrink-0 border-l border-slate-200 bg-white flex flex-col h-full">
      <div className="p-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <TypeIcon />
          <span className="text-xs font-bold text-slate-800">Column Inspector</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-5">
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Identity</div>
          <div className="text-sm font-mono font-bold text-slate-900 mb-1">{column.column_name}</div>
          <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-slate-200 text-slate-500">{column.data_type || 'unknown'}</span>
        </div>

        {/* Flags */}
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 space-y-3">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Behavior</div>
          <div className="flex items-center justify-between text-xs text-slate-700">
            ...
            <button onClick={() => setForm(f => ({ ...f, is_filterable: !f.is_filterable }))} className="transition-colors">
              {form.is_filterable ? <ToggleRight size={22} className="text-brand" /> : <ToggleLeft size={22} className="text-slate-300" />}
            </button>
          </div>
          <div className="flex items-center justify-between text-xs text-slate-700">
            ...
            <button onClick={() => setForm(f => ({ ...f, is_groupable: !f.is_groupable }))} className="transition-colors">
              {form.is_groupable ? <ToggleRight size={22} className="text-brand" /> : <ToggleLeft size={22} className="text-slate-300" />}
            </button>
          </div>
          <div className="flex items-center justify-between text-xs text-slate-700">
            <span className="font-medium">Visible</span>
            <button onClick={() => setForm(f => ({ ...f, is_visible: !f.is_visible }))} className="transition-colors">
              {form.is_visible ? <ToggleRight size={22} className="text-brand" /> : <ToggleLeft size={22} className="text-slate-300" />}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Friendly Name</label>
            <input value={form.friendly_name} onChange={e => setForm(f => ({ ...f, friendly_name: e.target.value }))}
              className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Description</label>
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3}
              className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none resize-none" placeholder="Business context for this column..." />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Format String</label>
            <input value={form.format_string} onChange={e => setForm(f => ({ ...f, format_string: e.target.value }))} placeholder="e.g. $#,##0.00"
              className="w-full px-3 py-2 text-xs font-mono border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Expression (Calculated)</label>
            <textarea value={form.expression} onChange={e => setForm(f => ({ ...f, expression: e.target.value }))} rows={2} placeholder="UPPER(first_name) || ' ' || last_name"
              className="w-full px-3 py-2 text-xs font-mono bg-slate-900 text-emerald-400 border-none rounded-lg outline-none focus:ring-2 focus:ring-brand resize-none" />
          </div>
        </div>
      </div>
