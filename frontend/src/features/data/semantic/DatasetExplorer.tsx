...
import { Database, Search, RefreshCcw, Columns, Hash } from 'lucide-react';

interface Dataset {
  id: number; name: string; table_name: string | null;
  columns: any[]; metrics: any[];
}
      ...
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-colors"
          />
        </div>
      </div>
        ...
        {filtered.map(ds => (
          ...
          >
            ...
            <div className="flex items-center gap-3 mt-2 ml-8">
              <span className="flex items-center gap-1 text-[9px] font-bold text-slate-400 dark:text-slate-500"><Columns size={10} />{ds.columns.length}</span>
              <span className="flex items-center gap-1 text-[9px] font-bold text-emerald-500 dark:text-emerald-400"><Hash size={10} />{ds.metrics.length}</span>
            </div>
          </button>
        ))}
      </div>
